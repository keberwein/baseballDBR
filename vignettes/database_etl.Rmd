...
The data contained in the Baseball Databank / Lahman Database are small enough to fit directly into memory using R. However, many may find it preferable to store the data in a local database to be accessed at a later time. There are two primary methods to accomplish this; extract transform load (ETL), and extract load transform (ELT).

The following examples use a local PostgreSQL database and the `RPostgreSQL` package. Many of the following methods would also work with other database connections such as the `RMySQL` and `DBI` packages.

## ETL

The goal here is to extrac the data from source, do the transformations in R, and load them into the database. With the `baseballDBR` and `RPostgreSQL` packages, this is a simple task. The following assumes we are starting with an empty Postgres instance named "lahman."
...
Once our data are loaded into a local database instance, working with the data becomes less cumbersome because we have the choice to only load into R the data that we neeed, or the choice to leverage `dplyr` or `SQL` backends to work with the data without loading into R at all.

### Using dplyr Backends

The `dplyr` package allows us to work with remote or on-disk data stored in databases using a familiar syntax and `dplyr` verbs. For more on database backends, see the [Databases vignette](https://cran.r-project.org/web/packages/dplyr/vignettes/databases.html) in the dplyr package. A current limitation to this method is, functions from the `baseballDBR` package will not work, and the method is limited to `dplyr` verbs.
...
pgcon <- src_postgres(host= "localhost", dbname= "lahman", user= "kriseberwein", password = "pissoff")
...
For those familiar with SQL, the same process can be acheived in our relational databases's native language.
